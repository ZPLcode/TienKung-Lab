...
class AMPPPO:
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(
            ...
            # 3.10.4 梯度惩罚（Gradient Penalty）— 判别器的"刹车"
            # ============================================================
            # 公式：grad_pen = λ × ||∇D(x)||²，只对专家数据计算
            #
            ...
            #     expert_d → +1.0
            #   导致 amp_reward = 0.3 × clamp(1 - 0.25×(d-1)², min=0)
            #                  = 0.3 × clamp(1 - 0.25×4, min=0)
            #                  = 0 ❌
            #   策略全程拿不到 AMP 奖励，AMP 信号死亡，训练停滞。
            #
            # grad_pen 怎么阻止这种情况：
            #   ||∇D(x)||² 衡量"输入变化一点，输出变化多少"
            #   惩罚梯度大 → 强迫判别器输出曲面平滑（不许有"决策悬崖"）
            #   判别器面临权衡：
            #     目标1: 把 policy_d 推到 -1（降 amp_loss）
            #     目标2: 保持输出平滑（降 grad_pen）
            #   两者妥协 → 判别器输出停在 ±0.5 而非 ±1 → AMP 信号正常工作
            #
            # λ 值的影响：
            #   λ=0（无惩罚）  → 判别器极度自信 → AMP 信号死锁
            #   λ=10（默认值） → 温和，判别器输出 ±0.4~0.5，工作正常
            #   λ=5（本次实验）→ 略锐利，输出 ±0.5~0.6，判别器更健康但收敛略慢
            #   λ=100（过强）  → 判别器太弱，输出 ±0.1，几乎区分不开
            #
            # [实验记录] 在 walk 任务上做过 λ=5 vs λ=10 对比：
            #   - 判别器分离度从 0.86 提升到 1.00（更健康）
            #   - 但最终 reward 持平（65.7 vs 64.6），训练时间多 3 倍
            #   - 视觉对比步态也看不出差别
            #   结论：默认 λ=10 在简单任务下已是合理选择
            #   详见 docs/amp_ppo_training_pipeline.md §15
            # 已恢复默认值 λ=10
