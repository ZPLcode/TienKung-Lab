...
class Discriminator(nn.Module):
    ...
    def compute_grad_pen(self, expert_state, expert_next_state, lambda_=10):
        """
        计算梯度惩罚（Gradient Penalty）— 判别器的"刹车"。

        公式：grad_pen = λ × ||∇D(x)||²，只对专家数据 x 计算
        ...
        grad_pen 如何阻止：
            ...
            妥协结果：判别器输出 ±0.5 而非 ±1
                → policy_d=-0.5 时 amp_reward=0.13（不为 0）✓
                → 策略持续收到学习信号

        ================ λ 值的影响 ================
            λ=0  （无惩罚）→ 判别器自信 → AMP 死锁
            λ=10 （默认）  → 温和，输出 ±0.4~0.5，工作正常
            λ=5  （实验）  → 略锐利，输出 ±0.5~0.6，判别器更健康
            λ=100（过强）  → 判别器太弱，几乎区分不开

        ================ 为什么只对专家数据计算 ================
            1. WGAN-GP 传统：在真实数据流形附近平滑判别器
            2. 节省计算：只算一次梯度，不算策略侧
            3. 实践证明：单边惩罚足够稳定训练

        ================ 类比理解 ================
            判别器 = 评分老师，policy = 学生
                无 grad_pen：老师只给 0 分或 100 分（学生学不到改进方向）
                有 grad_pen：老师给 30 分并指出问题（学生能改进）
                过强 grad_pen：老师都给 50 分（反馈太模糊）
